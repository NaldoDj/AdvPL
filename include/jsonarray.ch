 ...
 *
 * The jsonhash class is an adapted version of Marinaldo de Jesus' THASH Class.
 *
 *----------------------------------------------------------------------*
 *
 * -- How to install this program:
 *
 * 1 - Add the file jsonhash.prg to your project.
 *
 * 2 - Compile jsonhash.prg.
 *
 ...
 * aaFriends := jsonhash():New()
 * aaFriends:Set("David", jsonhash():New())
 * aaFriends:Get("David"):Set("Account", 187204)
 *
 * The jsonhash object has the aData array that stores all the data, so you
...
#IFNDEF _JSONARRAY_CH

    #DEFINE _JSONARRAY_CH
	...
	#IFNDEF __HARBOUR__
		...
		#xtranslate \[\#<k>\]               => :Get(<k>)
		#xtranslate Array(\#)               => JSONArray():New(.F.,.F.)
        #xtranslate ArrayFile(\#)           => JSONArray():New(.F.,.T.)
	#ELSE
		#xtranslate \[\#<k>\] := <v>        => \[<k>\] := <v>
		#xtranslate \[\#<k>\]               => \[<k>\]
		#xtranslate Array(\#)               => \{\=\>\}
